...
function ScheduleView({ appointments, setAppointments, clients, setClients }) {
  ...
  async function handleSave({ duration, name, phone, notes, wantsWhatsapp, wantsSms }) {
    playChime("booked");
    const appt = {
  ...
  if (quickAddStart != null) {
    return <QuickAddPanel dateKey={dateKey} startMin={quickAddStart} dayAppts={dayAppts} onCancel={() => setQuickAddStart(null)} onSave={handleSave} />;
  }
  ...
  return (
    <button onClick={onClick} className="w-full flex items-start gap-3 px-3 py-2 border-b last:border-b-0 text-left"
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-xs mt-0.5 break-words" style={{ color: C.inkMuted }}>{formatDuration(appt.duration)}{done ? ` · ${fmtLKR(appt.price)}` : ""}</div>
      </div>
...
function QuickAddPanel({ dateKey, startMin, dayAppts, onCancel, onSave }) {
  ...
  return (
    <div className="p-5">
      ...
      <div className="mt-6">
        <PrimaryButton full disabled={!canSave} onClick={() => onSave({ duration, name, phone, notes, wantsWhatsapp, wantsSms })}>Save appointment</PrimaryButton>
      </div>
    </div>
  );
}
